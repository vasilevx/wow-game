 ...
 *
 * @param newAud       New Audio to add
 *
 */
static void addMusic(Audio * root, Audio * newAud);
 ...
 *
 */
static void addAudio(Audio * root, Audio * newAud);
...
Audio * createAudio(const char * filename, uint8_t loop, int volume)
{
    ...
    if(SDL_LoadWAV(filename, &(newAud->audio), &(newAud->bufferTrue), &(newAud->lengthTrue)) == NULL)
    {
    ...
    newAud->buffer = newAud->bufferTrue;
    newAud->length = newAud->lengthTrue;
    (newAud->audio).callback = NULL;
    (newAud->audio).userdata = NULL;
...
static inline void playAudio(const char * filename, Audio * audio, uint8_t loop, int volume)
{
    ...
    if(filename != NULL)
    {
        /* Create newAud music sound with loop */
        newAud = createAudio(filename, loop, volume);
    }
    else if(audio != NULL)
    {
        newAud = new Audio;
        ...
        memcpy(newAud, audio, sizeof(Audio));
    ...
    if(loop == 1)
    {
        addMusic((Audio *) (gDevice->want).userdata, newAud);
    }
    else
    {
        addAudio((Audio *) (gDevice->want).userdata, newAud);
    }
...
static void addMusic(Audio * root, Audio * newAud)
{
    ...
    while(rootNext != NULL)
    {
        ...
        /* Set flag to remove any queued up music in favour of newAud music */
